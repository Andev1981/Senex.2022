...
export default function TableInsurances({
  ...
  const columns = useMemo(() => {
          ...
          cell: ({ row, getValue }) => (
              <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                      <p className="text-sm font-black text-gray-900 uppercase tracking-tight truncate leading-none mb-1">{getValue()?.replace(/^\d+\s-\s/, '')}</p>
                      <p className="text-[9px] font-black text-brand-gray opacity-60 uppercase tracking-widest font-mono">{row.original.rut}</p>
                  </div>
              </div>
          )
      },
          ...
          cell: ({ row }) => (
              <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 text-[10px] font-bold text-gray-600 truncate max-w-[180px]">
                      <Mail className="w-3 h-3 text-brand-primary opacity-40" /> {row.original.email || 'Sin Correo'}
                  </div>
                  <div className="flex items-center gap-2 text-[10px] font-bold text-gray-600">
                      <Phone className="w-3 h-3 text-brand-primary opacity-40" /> {row.original.phone || '---'}
                  </div>
              </div>
          )
      },
          ...
          cell: ({ row }) => {
              ...
              return (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded-xl border border-gray-100 w-fit">
                      ...
                      <span className="text-[10px] font-black text-gray-700 uppercase tracking-widest">{count} Planes</span>
                  </div>
              );
          }
      },
    ...
    return baseColumns;
  }, [handleOpenModalEdit, handleOpenModalDelete, openPlanListModal, user]);
      ...
      <div className="bg-white border border-gray-100 shadow-xl rounded-enterprise-xl overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-50 bg-gray-50/30">
          <h2 className="flex items-center gap-3 text-sm font-black text-gray-900 uppercase tracking-tight">
            <BrickWallShield className="w-5 h-5 text-brand-primary" /> Nómina de Previsión
          </h2>
                ...
                <tr>
                  <td colSpan={columns.length} className="px-8 py-24 text-center">
                    <div className="flex flex-col items-center justify-center space-y-4">
                        ...
                        <p className="enterprise-label opacity-40">No se detectaron aseguradoras vinculadas</p>
                    </div>
                  </td>
                </tr>
              ) : (
