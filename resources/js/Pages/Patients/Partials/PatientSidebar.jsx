...
export default function PatientSidebar({ patient, activeTab, setActiveTab, handleEditPatient }) {
  ...
  return (
    <aside className="flex flex-col w-full h-full min-h-screen overflow-y-auto border-r border-gray-200 bg-slate-50/80 md:w-80 custom-scrollbar backdrop-blur-sm">
      {/* 1. Perfil del Paciente (Estilo Ficha Técnica) */}
      <div className="relative flex flex-col items-center p-8 overflow-hidden text-center bg-white border-b border-gray-200">
        ...
        <div className="relative mb-6">
          ...
          <div
            className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-xl border-4 border-white shadow-sm ${
              patient.status === "active" ? "bg-green-500" : "bg-gray-300"
            }`}
          ></div>
        </div>

        <h2 className="text-xl font-black leading-tight tracking-tight text-gray-900 uppercase">
          {patient.name}{" "}
          <span className="block text-brand-primary">{patient.last_name}</span>
        </h2>
        <p className="mt-2 font-mono text-[10px] font-black text-brand-gray uppercase tracking-[0.2em]">
          {fmtRUT(patient.rut)}
        </p>
      </div>
      ...
      <nav className="flex-1 p-4 space-y-1">
        <p className="px-4 mb-4 enterprise-label opacity-40">
          Navegación Ficha
        </p>
        {menuItems.map((item) => {
      ...
      <div className="p-6 bg-white border-t border-gray-200">
        <button 
          ...
          className="w-full flex items-center justify-center gap-2 py-3 text-[9px] font-black uppercase tracking-widest text-brand-primary bg-brand-secondary/10 rounded-xl border border-brand-secondary/20 hover:bg-brand-primary hover:text-white transition-all duration-300"
        >
          <Edit className="w-3.5 h-3.5" /> Actualizar Perfil Maestro
        </button>
      </div>
    </aside>
  );
