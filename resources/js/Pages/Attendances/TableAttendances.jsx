...
export default function TableAttendances({
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) =>
          currencyFormatter.format(row.getValue("clinic_amount")),
      },
      {
        accessorKey: "status",
        header: "Estado",
  ...
  const exportToPDF = async () => {
    ...
    // Total de sessions (texto pequeño a la derecha)
    doc.setFontSize(9);
    const totalText = `Total de Sesiones: ${totalsessions}`;
    ...
    doc.save(`sessions-${currentYear}-${monthNum}.pdf`);
  };
          ...
          <tfoot>
            <tr className="font-semibold bg-gray-100">
              ...
              <td className="px-4 py-3" colSpan={4}>
                Totales (sessions: {totalsessions})
              </td>
              {/* Valor */}
